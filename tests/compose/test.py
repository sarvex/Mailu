...
test_path = f"tests/compose/{test_name}/"
compose_file = f"{test_path}docker-compose.yml"
...
def stop(exit_code):
    ...
    print(
        subprocess.check_output(
            f"docker-compose -f {compose_file} down", shell=True
        ).decode()
    )
    sys.exit(exit_code)
...
def sleep():
    print(f"{Fore.LIGHTMAGENTA_EX}Sleeping for {str(timeout)}m{Style.RESET_ALL}")
    ...
    for container in client.containers(all=True):
        ...
        if "Health" in container_inspect['State'].keys():
            if container_inspect['State']['Health']['Status'] == "healthy":
                print(
                    f"{Fore.GREEN}Health status for "
                    + container_inspect['Name'].replace("/", "")
                    + " : "
                    + Fore.CYAN
                    + container_inspect['State']['Health']['Status']
                    + Style.RESET_ALL
                )
            if container_inspect['State']['Health']['Status'] != "healthy":
                print(
                    (
                            ...
                            (
                                (
                                    (
                                        (
                                            (
                                                f"{Fore.RED}Container "
                                                + container_inspect[
                                                    'Name'
                                                ].replace("/", "")
                                                + " is "
                                                + Fore.YELLOW
                                                + container_inspect['State'][
                                                    'Health'
                                                ]['Status']
                                                + Fore.RED
                                            )
                                    ...
                                    + str(
                                        container_inspect['State']['Health'][
                                            'FailingStreak'
                                        ]
                                    )
                                    + Fore.RED
                        ...
                        + str(container_inspect['State']['Health']['Log'])
                        + Style.RESET_ALL
                    )
                )
                exit_code = 1
            ...
            if container_inspect['State']['Status'] == "running":
                print(
                    f"{Fore.GREEN}Running status for "
                    + container_inspect['Name'].replace("/", "")
                    + " : "
                    + Fore.BLUE
                    + container_inspect['State']['Status']
                    + Style.RESET_ALL
                )
            if container_inspect['State']['Status'] != "running":
                print(
...
def print_logs():
    ...
    for container in containers:
        print(
            f"{Fore.LIGHTMAGENTA_EX}Printing logs for: {Fore.GREEN}"
            + container['Name']
            + Style.RESET_ALL
        )
        sys.stdout.flush()
...
def hooks():
    print(f"{Fore.LIGHTMAGENTA_EX}Running hooks{Style.RESET_ALL}")
    for test_file in sorted(os.listdir(test_path)):
        try:
            if test_file.endswith(".py"):
                ...
                print(
                    subprocess.check_output(
                        f"python3 {test_path}{test_file}", shell=True
                    ).decode()
                )
                ...
                print(
                    subprocess.check_output(
                        f"./{test_path}{test_file}", shell=True
                    ).decode()
                )
...
print(
    subprocess.check_output(
        f"docker-compose -f {compose_file} up -d", shell=True
    ).decode()
)
print()
sleep()
